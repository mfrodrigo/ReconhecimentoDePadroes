...
<<echo=False>>=
set.seed(12)
index <- sample(1:nrow(data), length(1:nrow(data)))
x <- data[index,]
...
accuracy_1 <- sum(1*(x_test[,14] == y_hat))/ nrow(x_test)
...
x_test <- x[(training_sample_number+1):nrow(x), ]
y_hat <- bayes_classifier(x_train, x_test)
accuracy_2 <- sum(1*(x_test[,14] == y_hat))/ nrow(x_test)

index <- sample(1:nrow(data), length(1:nrow(data)))
x <- data[index,]
training_sample_number = round(nrow(x)*0.2)
x_train <- x[1:training_sample_number,]
x_test <- x[(training_sample_number+1):nrow(x), ]
y_hat <- bayes_classifier(x_train, x_test)
accuracy_3 <- sum(1*(x_test[,14] == y_hat))/ nrow(x_test)
@
