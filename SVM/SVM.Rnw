...
<<fig = True, echo=False>>=
plot(data)
points(x_train[SVindex(svm), ], col = "green")
@
...
\centering
<<fig = True, echo=False>>=
# Plotar dados com hiperplano de separação
x1seq <- seq(-1.2, 1.2, 0.1)
x2seq <- seq(-1.2, 1.2, 0.1)
...
for(i in 1:length(x1seq))
{
  for(j in 1:length(x2seq))
  {
    x1 <- x1seq[i]
    x2 <- x2seq[j]
    xin <- matrix(c(x1,x2), nrow = 1)
    M[i,j] = predict(svm, xin)
  }
}

plot(x[ ,1], x[ ,2], col = 1*(y>=0)+1, xlim = c(-1.2,1.2), ylim = c(-1.2,1.2))
par(new = T)
contour(x1seq, x2seq, M, nlevels = 1, xlim = c(-1.2,1.2), ylim = c(-1.2,1.2))
@
...
<<fig = True, echo=False>>=
library('plot3D')
persp3D(x1seq, x2seq, M, counter=T, theta = 55, phi = 30, r = 40, d = 0.1, expand = 0.5,
